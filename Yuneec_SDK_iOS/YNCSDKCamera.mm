...
void receive_cameraOperate_error(YNCCompletionBlock completion, Camera::Result result) {
    if (completion) {
        NSError *error = nullptr;
        if (result != Camera::Result::SUCCESS) {
            NSString *message = [NSString stringWithFormat:@"%s", Camera::result_str(result)];
            error = [[NSError alloc] initWithDomain:@"CameraOperate"
                                               code:(int)result
...
void receive_cameraGetSetting_error(YNCReceiveDataCompletionBlock completion, Camera::Result result, Camera::Settings settings) {
    if (completion) {
        NSError *error = nullptr;
        if (result != Camera::Result::SUCCESS) {
            NSString *message = [NSString stringWithFormat:@"%s", Camera::result_str(result)];
...
+ (void)takePhotoWithCompletion:(YNCCompletionBlock)completion {
    ...
    dl->device().camera().take_photo_async(std::bind(&receive_cameraOperate_error, completion, _1));
}

//MARK: Camera StartVideo
+ (void)startVideoWithCompletion:(YNCCompletionBlock)completion {
    DroneLink *dl = [[YNCSDKInternal instance] dl];
    dl->device().camera().start_video_async(std::bind(&receive_cameraOperate_error, completion, _1));
}

//MARK: Camera StopVideo
+ (void)stopVideoWithCompletion:(YNCCompletionBlock)completion {
    DroneLink *dl = [[YNCSDKInternal instance] dl];
    dl->device().camera().stop_video_async(std::bind(&receive_cameraOperate_error, completion, _1));
}

//MARK: Camera StartPhotoInterval
+ (void)startPhotoInterval:(double)intervalS Completion:(YNCCompletionBlock)completion{
    DroneLink *dl = [[YNCSDKInternal instance] dl];
    dl->device().camera().start_photo_interval_async(intervalS, std::bind(&receive_cameraOperate_error, completion, _1));
}

//MARK: Camera StopPhotoInterval
+ (void)stopPhotoIntervalWithCompletion:(YNCCompletionBlock)completion {
    DroneLink *dl = [[YNCSDKInternal instance] dl];
    dl->device().camera().stop_photo_interval_async(std::bind(&receive_cameraOperate_error, completion, _1));
}
...
+ (void)setSettings:(YNCCameraSettings *)cameraSettings Completion:(YNCCompletionBlock)completion {
    ...
    dl->device().camera().set_settings_async(settings, std::bind(&receive_cameraOperate_error, completion, _1));
}

//MARK: Camera get settings
+ (void)getSettings:(YNCReceiveDataCompletionBlock)receiveDataCompletion {
    DroneLink *dl = [[YNCSDKInternal instance] dl];
    dl->device().camera().get_settings_async(std::bind(&receive_cameraGetSetting_error, receiveDataCompletion, _1, _2));
}
